...
class DeepSeekClient:
    ...
    def __init__(self, config: dict):
        ...
        # New configuration
        self.formal_timeout: float = float(self.config.get("formal_answer_timeout_seconds", 60))
        self.stage_mode: str = (self.config.get("analysis_stage_mode") or "multi").strip().lower()
    ...
    def _send_multi_stage(self, text: str) -> None:
        ...
        memory_block = "" if not (memory_topics or memory_bg) else (
            "\n\nKnown memory to refine (topics/background):\n"
            + ("topics: " + ", ".join(memory_topics) + "\n" if memory_topics else "")
        ...
        preflight_system = (
            ...
            "5) Model recommendation policy: default to 'chat'; choose 'reasoner' whenever the task benefits from structured multi-step reasoning, decomposition, or formal justification. For genuinely complex or high-ambiguity problems, DO NOT choose 'chat'/'fast'; misclassifying a complex case as 'chat' is a serious error. Provide brief 'why_model'.\n"
            "6) Provide 'confidence' in [0,1] (use high values only when very certain; if uncertain, prefer 'chat').\n"
        ...
        for attempt in range(1, self.retry_attempts + 1):
            if attempt > 1:
                reason_text = (
                    {"timeout": "超时", "network": "网络错误", "http": "HTTP 错误", "unknown": "未知错误"}.get(self._last_error_kind or "", "未知")
                    if self.i18n.lang_code == "zh"
                    else {"timeout": "timeout", "network": "network error", "http": "HTTP error", "unknown": "unknown error"}.get(self._last_error_kind or "", "unknown")
                )
                console.print(self.i18n.t("deepseek.retrying", stage="Preprocessing", attempt=str(attempt), total=str(self.retry_attempts), reason=reason_text))
            result = self._send_request(
                self._url, self._headers, preflight_payload, "preprocessing", self.fast_model
            )
            if not result:
                continue
            preflight_data, _elapsed1 = result
            preflight_msg = (
                (preflight_data or {}).get("choices", [{}])[0].get("message", {}).get("content", "")
            )
        ...
        if not preflight_json:
            ...
            if content_type != "question":
                ...
                content_type_str = ("非题目" if self.i18n.lang_code == "zh" else "Non-question")
                topics = preflight_json.get("related_topics") or []
                background = preflight_json.get("background_knowledge") or ""
                topics_str = ", ".join([t for t in topics if isinstance(t, str) and t.strip()]) or self.i18n.t("deepseek.no_content")
            ...
            recommended_model = (preflight_json.get("recommended_model") or "reasoner").lower()
            ...
            options_lines = "\n".join(
                [f"{opt.get('label')}. {opt.get('text')}" for opt in options if isinstance(opt, dict)]
            )
            model_name = (
            ...
            confidence_str = confidence if confidence is not None else self.i18n.t("deepseek.not_available")
        ...
        use_reasoner = (
            recommended_model == "reasoner" and isinstance(suggest_len, (int, float)) and int(suggest_len) >= 128
        )
        ...
        console.print(self.i18n.t("deepseek.answering_model", model=answer_model, type=type_name))

        lang_token = "Chinese" if self.output_language == "zh" else "English"
        memory_bg_block = "\n\nBackground knowledge (for reference only):\n" + (
            "\n".join(self.background_knowledge[-3:]) if self.background_knowledge else background
        )
        thinking_hint = (
        ...
        answer_result = self._send_request(
            self._url, self._headers, answer_payload, "formal answering", answer_model, timeout_override=self.formal_timeout
        )
        ...
        if not answer_result and answer_model != self.fast_model:
            console.print(
                Panel(
                    Text(self.i18n.t("deepseek.error_timeout_body", stage="Formal", error=self.i18n.t("deepseek.not_available"))) ,
            ...
            answer_result = self._send_request(
                self._url, self._headers, answer_payload, "formal answering (fallback)", self.fast_model, timeout_override=self.formal_timeout
            )
        ...
        if usage:
            usage_text = Text()
            usage_text.append(self.i18n.t("deepseek.process_usage_title"), style="bold blue")
            usage_text.append(f"prompt_tokens={usage.get('prompt_tokens')}, ", style="cyan")
            usage_text.append(f"completion_tokens={usage.get('completion_tokens')}, ", style="cyan")
            usage_text.append(f"total_tokens={usage.get('total_tokens')}", style="cyan")
            console.print(usage_text)

        console.print(self.i18n.t("deepseek.parse_answer"))
        answer_msg = (
            (answer_data or {}).get("choices", [{}])[0].get("message", {}).get("content", "")
        )
        ...
        if not result_json:
            ...
            console.print(
                Panel(pretty, title=f"[green]{self.i18n.t('deepseek.formal_answer_title')}[/green]", border_style="green")
            )
    ...
    def _send_single_stage(self, text: str) -> None:
        ...
        for attempt in range(1, self.retry_attempts + 1):
            if attempt > 1:
                ...
                console.print(self.i18n.t("deepseek.retrying", stage="Single-Stage", attempt=str(attempt), total=str(self.retry_attempts), reason=reason_text))
            result = self._send_request(
                self._url, self._headers, payload, "single-stage", current_model, timeout_override=self.formal_timeout
            )
            if not result:
                if current_model != self.fast_model and self._last_error_kind == "timeout":
                    current_model = self.fast_model
                    payload["model"] = self.fast_model
                continue
            data, _elapsed = result
            msg = (data or {}).get("choices", [{}])[0].get("message", {}).get("content", "")
        ...
        choice_type = (review.get("choice_type") or review.get("question_kind") or "none").lower()
    ...
    def parse_json_strict(self, text: str) -> Optional[dict]:
    ...
    def _send_request(
        ...
        console.print(self.i18n.t("deepseek.sending_request", stage=stage_name, model=model))
        start_t = time.perf_counter()
        resp = None
        data: Any = None
        try:
            with console.status(f"[bold blue]{self.i18n.t('deepseek.waiting_response', stage=stage_name)}"):
                timeout = float(timeout_override) if timeout_override is not None else (
                    120.0 if "reason" in model else 90.0
                )
                resp = self.client.post(url, json=payload, headers=headers, timeout=httpx.Timeout(timeout))
            ...
            console.print(
                Panel(
                    self.i18n.t("deepseek.error_timeout_body", stage=stage_name.title(), error=str(e)),
                    title=f"[red]{self.i18n.t('deepseek.error_timeout_title')}[/red]",
                    border_style="red",
                )
            )
            ...
            console.print(
                Panel(
                    self.i18n.t("deepseek.error_network_body", stage=stage_name.title(), error=str(e)),
            ...
            console.print(
                Panel(
                    self.i18n.t("deepseek.error_http_body", stage=stage_name.title(), error=str(e), body=body),
            ...
            console.print(
                Panel(
                    self.i18n.t("deepseek.error_unknown_body", stage=stage_name.title(), error=str(e)),
